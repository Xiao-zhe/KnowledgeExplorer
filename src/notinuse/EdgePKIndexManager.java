...
import edu.whu.clock.newprobindex.PKIndexTypedEntryBinding;

public class EdgePKIndexManager {
	...
	public EdgePKIndexManager(String dbEnvPath) {
	...
	public void build(String dir, ClassManager classManager,
		...
		HashMap<String, TreeMap<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>>> map = new HashMap<String, TreeMap<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>>>();
		try {
			BufferedReader br = new BufferedReader(new FileReader(dir
					+ "/instance_attribute_mapping.txt"));
			String line = null;
...
//					HashSet<IndexedEdgeTyped> indexEdges = new HashSet<IndexedEdgeTyped>(); 
					TreeMap<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>> 
					   abEdge = new TreeMap<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>>();                                                                        // To
																							// collect
																							// all
																							// unique
																							// index
					...
					for (EntityGraphEdgeTyped edge : neighborEdges) {
						IndexedEdgeTyped indexEdge = new IndexedEdgeTyped(classID,  /////////////////////////////////////////////////////////
								instanceManager.getClassID(edge.getEnd()), edge
										.getType(), edge.isOut());
						if(abEdge.containsKey(indexEdge)){
							abEdge.get(indexEdge).add(edge);
						}
						else{
							HashSet<EntityGraphEdgeTyped> edges = new HashSet<EntityGraphEdgeTyped>();
							edges.add(edge);
							abEdge.put(indexEdge, edges);
						}
					...
					for (String term : termSet) {////????????????????????????????????????????????????????????????????????????????????????
						if (map.containsKey(term)) {
							TreeMap<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>> edgeMap = map
									.get(term);
							for (IndexedEdgeTyped edge : abEdge.keySet()) {
								if (edgeMap.containsKey(edge)) {
					                 edgeMap.get(edge).addAll(abEdge.get(edge));          
								} else {
									edgeMap.put(edge, abEdge.get(edge));
								}
							}
							// map.put(term, edgeMap);
						} else {
			...
			for (String term : map.keySet()) {
				...
				DatabaseEntry key = new DatabaseEntry(term.getBytes("UTF-8"));
				TreeMap<IndexedEdgeTyped,HashSet<EntityGraphEdgeTyped> > ei = null;
				ei = map.get(term);
				IndexedEdgeTyped[] edgeList = new IndexedEdgeTyped[ei.size()];
				Iterator<Entry<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>>> iter = ei.entrySet()
						.iterator();
				int i = 0;
				while (iter.hasNext()) {
					Entry<IndexedEdgeTyped, HashSet<EntityGraphEdgeTyped>> entry = iter.next();
					IndexedEdgeTyped k = entry.getKey();
					double val = entry.getValue().size();
//					double vnum = classManager.getInstanceNum(k.getStart());
					int classB = k.getEnd();
					int vnum = 0;
					for(int ins:instanceManager.getInstanceSet(k.getStart())){
						if(entityGraph.haveEdgeBetweenInstanceAndClass(ins, classB)){
							vnum++;
						}
					}
					k.setProb(val / vnum);
