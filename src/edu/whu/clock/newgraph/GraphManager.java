...
//		summaryGraphTyped.write2file(GraphManager.SERIALIZED_FILE_DIR);
	...
	public void genEntityGraphTyped(){
		...
		Date start = new Date();
		entityGraphTyped = new EntityGraphTyped(instanceManager);
	...
	public void genPKIndexManager(boolean typed) {
		Date start = new Date();
		pkIndex = new PKIndexManager(GraphManager.INDEX_DB_DIR);
		if (typed) {
			pkIndex.build(GraphManager.GRAPH_FILE_DIR, classManager, instanceManager, entityGraphTyped);
		}
		else {
			pkIndex.build(GraphManager.GRAPH_FILE_DIR, classManager, instanceManager, entityGraph);
		}
		Date end = new Date();
		System.out.println("Generating the typed PK-Index is finished.");
		System.out.println("Time exhausted: " + (end.getTime()-start.getTime()) + " msec");
//		pkIndex.close();
	}
	...
	public void initPKIndexManager() {
		Date start = new Date();
		pkIndex = new PKIndexManager(GraphManager.INDEX_DB_DIR);
		Date end = new Date();
		System.out.println("Initializing the PK-Index is finished.");
		System.out.println("Time exhausted: " + (end.getTime()-start.getTime()) + " msec");
	}
